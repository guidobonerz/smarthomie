package de.drazil.spring.scheduling;
...
public class ScheduledAnnotationBeanPostProcessor2
		implements BeanPostProcessor, Ordered, EmbeddedValueResolverAware, BeanFactoryAware, ApplicationContextAware,
		SmartInitializingSingleton, ApplicationListener<ContextRefreshedEvent>, DisposableBean {

	/**
	 * The default name of the {@link TaskScheduler} bean to pick up:
	 * "taskScheduler".
	 * <p>
	 ...
	 * {@link SchedulingConfigurer} beans won't get autodetected and a
	 * {@link #setScheduler scheduler} has to be explicitly configured.
	 */
	@Override
	 ...
	 * tasks will be activated in the {@link ContextRefreshedEvent} phase; if
	 * not set, it will happen at {@link #afterSingletonsInstantiated} time.
	 */
	@Override
	...
	public void onApplicationEvent(ContextRefreshedEvent event) {
		if (event.getApplicationContext() == this.applicationContext) {
			...
			// their work at the same time (e.g. Spring Batch's job
			// registration).
			finishRegistration();
		}
	}
	...
	private void finishRegistration() {
		...
		if (this.beanFactory instanceof ListableBeanFactory) {
			Map<String, SchedulingConfigurer> configurers = ((ListableBeanFactory) this.beanFactory)
		...
		if (this.registrar.hasTasks() && this.registrar.getScheduler() == null) {
			...
			try {
				...
				try {
					...
					// Giving up -> falling back to default scheduler within the
					// registrar...
				}
			}
		}
			...
			Map<Method, Set<Scheduled>> annotatedMethods = MethodIntrospector.selectMethods(targetClass,
					new MethodIntrospector.MetadataLookup<Set<Scheduled>>() {
						...
						public Set<Scheduled> inspect(Method method) {
							Set<Scheduled> scheduledMethods = AnnotationUtils.getRepeatableAnnotations(method,
	...
	protected void processScheduled(Scheduled scheduled, Method method, Object bean) {
		try {
			...
			if (AopUtils.isJdkDynamicProxy(bean)) {
				try {
					// Found a @Scheduled method on the target class for this
					// JDK proxy ->
				...
				// Common problem: private methods end up in the proxy instance,
				// not getting delegated.
			...
			if (StringUtils.hasText(cron)) {
				...
				String singleCronTimer = cron;

				if (cron.indexOf("{") > 0 && cron.indexOf("}") > 0) {
					this.registrar.addCronTask(new CronTask(runnable, new CronTrigger(cron, timeZone)));
				} else {

					this.registrar.addCronTask(new CronTask(runnable, new CronTrigger(singleCronTimer, timeZone)));
				}
